...
class WC_Payever_Admin_Settings extends WC_Settings_Page {
	...
	public function __construct( $wp_wrapper = null ) {
		...
		$this->get_wp_wrapper()->add_action(
			'woocommerce_admin_field_payever_download_logs_button',
			array(
				$this,
				'payever_download_logs_button',
			)
		);
	...
	public function payever_download_logs_button() {
		?>
		<tr valign="top" class="">
		...
		return $this->get_wp_wrapper()->apply_filters(
			...
			array(
				...
				array(
					...
					'type'  => 'payever_download_logs_button',
				),
				array(
					'title'             => __( 'Company Search Enabled', 'payever-woocommerce-gateway' ),
					'id'                => self::PAYEVER_OPTION_PREFIX . '_enable_company_search',
					'css'               => $this->element_width . 'opacity:0.5;',
					'type'              => 'select',
	...
	public function save() {
		...
		foreach ( $settings as $index => $item ) {
			if ( self::PAYEVER_OPTION_PREFIX . '_enable_company_search' === $item['id'] ) {
