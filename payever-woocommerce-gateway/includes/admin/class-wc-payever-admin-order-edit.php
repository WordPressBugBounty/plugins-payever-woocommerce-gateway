...
class WC_Payever_Admin_Order_Edit {
	...
	/** @var array */
	private $is_claim_upload_allowed_for_order_id = array();

	/** @var array */
	private $is_claim_allowed_for_order_id = array();
	...
	public function add_buttons( $order ) {
		...
		<?php if ( $this->is_cancel_action_allowed( $order ) ) : ?>
			...
			?>
		<?php endif; ?>
		<?php if ( $this->is_claim_upload_action_allowed( $order ) ) : ?>
			<button type="button" class="button wc-payever-claim-upload-button" title="<?php esc_html_e( 'Claim upload', 'payever-woocommerce-gateway' ); ?>">
				<?php esc_html_e( 'Claim upload', 'payever-woocommerce-gateway' ); ?>
			</button>
			<?php
			wc_get_template(
				'admin/html-order-claim-upload.php',
				array(
					'order'    => $order,
					'order_id' => $order_id,
				),
			...
			?>
		<?php endif; ?>
		<?php if ( $this->is_claim_action_allowed( $order ) ) : ?>
			<button type="button" class="button wc-payever-claim-button" title="<?php esc_html_e( 'Claim', 'payever-woocommerce-gateway' ); ?>">
				<?php esc_html_e( 'Claim', 'payever-woocommerce-gateway' ); ?>
			</button>
			<?php
			wc_get_template(
				'admin/html-order-claim.php',
				array(
					'order'    => $order,
					'order_id' => $order_id,
				),
	 ...
	 * @throws Exception
	 */
	private function is_claim_upload_action_allowed( WC_Order $order ) {
		$order_id   = $order->get_id();
		$payment_id = $order->get_meta( WC_Payever_Gateway::PAYEVER_PAYMENT_ID );
		if ( empty( $payment_id ) ) {
			return false;
		...
		if ( ! isset( $this->is_claim_upload_allowed_for_order_id[ $order_id ] ) ) {
			try {
				$api = $this->get_api_wrapper()
					->get_payments_api_client();

				$action_decider = $this->get_action_decider_wrapper()->get_action_decider( $api );
				$this->is_claim_upload_allowed_for_order_id[ $order_id ] = $action_decider->isClaimUploadAllowed(
					$payment_id,
					false
				);
			} catch ( \Exception $exception ) {
				$this->get_api_wrapper()->get_logger()->error( $exception->getMessage() );
				$this->is_claim_upload_allowed_for_order_id[ $order_id ] = false;
			}
		}

		return $this->is_claim_upload_allowed_for_order_id[ $order_id ];
	}
	 ...
	 * @throws Exception
	 */
	private function is_claim_action_allowed( WC_Order $order ) {
		$order_id   = $order->get_id();
		$payment_id = $order->get_meta( WC_Payever_Gateway::PAYEVER_PAYMENT_ID );
		if ( empty( $payment_id ) ) {
			return false;
		...
		if ( ! isset( $this->is_claim_allowed_for_order_id[ $order_id ] ) ) {
			try {
				$api = $this->get_api_wrapper()
					->get_payments_api_client();

				$action_decider                                   = $this->get_action_decider_wrapper()->get_action_decider( $api );
				$this->is_claim_allowed_for_order_id[ $order_id ] = $action_decider->isClaimAllowed(
					$payment_id,
					false
				);
			} catch ( \Exception $exception ) {
				$this->get_api_wrapper()->get_logger()->error( $exception->getMessage() );
				$this->is_claim_allowed_for_order_id[ $order_id ] = false;
			}
		}

		return $this->is_claim_allowed_for_order_id[ $order_id ];
	}
}
