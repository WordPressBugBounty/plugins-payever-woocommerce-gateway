...
class WC_Payever_Payment_Service {
	...
	private function submit_payment( WC_Order $order ) {
		...
		if ( 'yes' === $this->settings['is_b2b_method'] ) {
			$this->set_customer_company( $order, $payment_parameters );
		}
	 ...
	 * @throws Exception
	 */
	private function set_customer_company( WC_Order $order, &$payment_parameters ) {
		$customer_company_service = new WC_Payever_Customer_Company();
		$company_data_model = $customer_company_service->get_item(
		...
		if ( empty( $company_data_model ) ) {
			throw new \Exception( 'Not valid billing company address. Please update billing company address.' );
		}

		$company_data = unserialize( $company_data_model['company'] );

		$company_entity = new CompanyEntity();
		$company_entity
			->setTaxId( $company_data['vat_id'] ?? null )
			->setExternalId( $company_data['id'] )
			->setName( $company_data['name'] );

		$payment_parameters->setCompany( $company_entity );
	...
	private function get_customer_entity( $order ) {
		...
		if ( ! empty( $company ) && 'yes' === $this->settings['is_b2b_method'] ) {
