...
class WC_Payever_Synchronization {
	 ...
	 * @throws Exception
	 */
	private static function sync_b2b_options() {
		update_option(
			WC_Payever_Helper::PAYEVER_B2B_COMPANY_SEARCH,
			WC_Payever_Api::get_instance()->get_payments_api_client()->isB2bSearchActive()
		);
	}
	...
	private static function prepare_payment_options( $paymentOptions ) {
		...
		foreach ( $converted_options as $payment_option ) {
			...
			$prepared_payment_options[ $payment_method ] = self::get_payment_options(
				$payment_option,
				$variantName,
				$origin_payment_method,
				$current_option
			...
			if ( isset( $prepared_payment_options[ $payment_method ]['is_submit_method'] ) ) {
				foreach ( $prepared_payment_options[ $payment_method ]['countries'] as $country ) {
					$b2b_countries[] = strtolower( $country );
				}
			}
		...
		WC_Payever_Helper::instance()->set_b2b_countries( $b2b_countries );
	...
	private static function get_payment_options( $payment_option, $variantName, $origin_payment_method, $current_option ) {
		...
		$options = array(
			...
			'is_b2b_method'    => ( $payment_option->isB2BMethod() ) ? 'yes' : 'no',
			'business_type'    => $payment_option->getBusinessType(),
		);
		...
		if ( $payment_option->isSubmitMethod() ) {
			$options['is_submit_method'] = 'yes';
		}
