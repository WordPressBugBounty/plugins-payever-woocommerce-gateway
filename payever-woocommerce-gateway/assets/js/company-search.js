...
const PayeverApiClient = {
    init(options) {
        ...
        if (options && options.companyRequestUrl) {
            this.companyFindEndpoint = options.companyRequestUrl + '&term={value}&country={country}';
        }

        this.companySaveEndpoint = '/payment/payever/company';

        if (options && options.companySaveRequestUrl) {
            this.companySaveEndpoint = options.companySaveRequestUrl;
        }
    },
...
const PayeverCountryPicker = {
    ...
    _generateMarkup() {
        ...
        this.countryInput.closest('form').classList.add('country-select-enabled');
    ...
    _bindDropdownListeners() {
        ...
        me.options.isOpening = !me.options.isOpening;
        me.clickEventHandler = me._click.bind(me);
        window.addEventListener('click', me.clickEventHandler);
        this.countryInput.addEventListener('click', me.clickEventHandler);
     ...
     */
    _handleEnterKey() {
        let highlighted = PayeverDomAccess.querySelector(this.countryList, 'li.highlight');

        if (highlighted) {
            this._selectListItem(highlighted);
        }
    },
    ...
    _selectListItem(listItem) {
        ...
        this.companySearch.fillCountry();
    ...
    _closeDropdown() {
        ...
        window.removeEventListener('click', me.clickEventHandler);
        window.removeEventListener('keydown', me.keydownEventHandler);
        this.countryInput.removeEventListener('click', me.clickEventHandler);
        this.typedLetters = '';
    },
...
const PayeverCompanyAutocomplete = {
    ...
    initAutocompleteDOM() {
        ...
        autocomplete.style.width = this.companyElement.parentNode.offsetWidth + "px";
...
const PayeverCompanySearch = {
    ...
    options: {
        selectorDefaultAddressFields: {
            street: '[name*=billing_address_1], #billing-address_1',
            zipcode: '[name*=billing_postcode], #billing-postcode',
            city: '[name*=billing_city], #billing-city',
            housenumber: '[name*=billing_address_2], #billing-address_2',
            countryId: '[name*=billing_country], #billing-country',
        },
        countryIdInput: '[name*=billing_country], #billing-country',
        /**
         * Specifies the element where the user types his company
         * @type string
         */
        selectorCompanyInput: '[name*=billing_company], #billing-company',
         ...
         * @type string
         */
        selectorHousenumber: '[name*=billing_address_2]',
    ...
    saveAddress() {
        ...
        const data = {
            company: formData.get('billing_company') || jQuery('#billing-company')?.val(),
            email: formData.get('billing_email') || jQuery('#billing-email')?.val(),
            town: formData.get('billing_city') || jQuery('#billing-city')?.val(),
            zip: formData.get('billing_postcode') || jQuery('#billing-postcode')?.val(),
    ...
    setElementValue(element, value)
    {
        ...
        if (element.tagName === 'DIV') {
            element = element.querySelector('input, select');
        }
...
const payeverCompanySearchInit = function() {
    const observer = new MutationObserver(function (mutations) {
        const inputs = PayeverDomAccess.querySelectorAll(null, PayeverCompanySearch.options.selectorCompanyInput);
        if (inputs.length) {
            inputs.forEach(function(element) {
                if (element.hasAttribute('payever-company-field')) {
                    return;
                }
                let PayeverCompanySearchInstance = Object.assign({}, PayeverCompanySearch);
                PayeverCompanySearchInstance.init(element, Payever_Company_Search.json_config);
                element.setAttribute('payever-company-field', true);
            });
        }
    });
